(() => {
  ...
  function createStatusToolbar(id, label, actions = '') {
    const toolbar = document.createElement('div');
    toolbar.id = id;
    toolbar.className = 'cloud-toolbar';
    toolbar.innerHTML = `<div class="cloud-status" data-cloud-status>${statusMarkup()}</div><div class="cloud-toolbar-actions">${actions || `<span class="sub">${label}</span>`}</div>`;
    return toolbar;
  }
  ...
  function installToolbars() {
    ...
    if (tripHead && !document.getElementById('tripCloudToolbar')) {
      const actions = '<button class="cloud-edit-btn secondary" id="addTripDay" type="button" hidden>＋ 新增一天</button><button class="cloud-edit-btn" id="itineraryEditToggle" type="button">編輯行程</button>';
      const toolbar = createStatusToolbar('tripCloudToolbar', '共同行程', actions);
    ...
    if (prepHead && !document.getElementById('prepCloudToolbar')) {
      const toolbar = createStatusToolbar('prepCloudToolbar', '共同清單');
    ...
    const budgetHead = document.querySelector('#budget .page-head');
    if (budgetHead && !document.getElementById('budgetCloudToolbar')) {
      const toolbar = createStatusToolbar('budgetCloudToolbar', '共同預算');
      const title = budgetHead.querySelector('.section-title');
      title ? title.insertAdjacentElement('afterend', toolbar) : budgetHead.prepend(toolbar);
    }
  }

  function setStatus(mode, text) {
    document.querySelectorAll('[data-cloud-status]').forEach(element => {
      element.classList.remove('online', 'syncing', 'error');
      if (mode) element.classList.add(mode);
      const label = element.querySelector('.cloud-status-text');
  ...
  function decodeEntryPayload(payload) {
    if (Array.isArray(payload)) {
      return { items: clone(payload), budget: null, hasBudget: false };
    }
    if (payload && typeof payload === 'object') {
      const budget = payload.budget && typeof payload.budget === 'object' ? payload.budget : null;
      return {
    ...
    return { items: [], budget: null, hasBudget: false };
  }

  function applyBudget(budget) {
    if (!budget || typeof budget !== 'object') return;
    const limit = Number(budget.budgetLimit);
    if (Number.isFinite(limit)) state.budgetLimit = Math.max(0, limit);
    if (Array.isArray(budget.expenses)) state.expenses = clone(budget.expenses);
  }

  function encodeEntryPayload() {
    return {
      version: sharedPayloadVersion,
      items: clone(Array.isArray(state.entry) ? state.entry : []),
      budget: {
        budgetLimit: Math.max(0, Number(state.budgetLimit) || 0),
  ...
  function snapshot() {
    return {
      ...
      entry_tasks: encodeEntryPayload(),
  ...
  function restoreLocalDraft() {
    try {
      ...
      const decoded = decodeEntryPayload(draft.entry_tasks);
      if (Array.isArray(decoded.items)) state.entry = decoded.items;
      if (decoded.hasBudget) applyBudget(decoded.budget);
      save();
    } catch {}
  }
  ...
  function applyRemote(row) {
    ...
    try {
      ...
      const decoded = decodeEntryPayload(row.entry_tasks);
      if (Array.isArray(decoded.items)) state.entry = decoded.items;
      if (decoded.hasBudget) applyBudget(decoded.budget);
  ...
  function installChecklistHooks() {
    ['toggleNote', 'renameNote', 'insertAfter', 'removeEmpty', 'appendBooking', 'appendEntryTask', 'appendPacking'].forEach(wrapFunction);
  }

  function installBudgetHooks() {
    document.getElementById('budgetLimit')?.addEventListener('change', scheduleCloudSave);
    document.getElementById('expenseForm')?.addEventListener('submit', () => setTimeout(scheduleCloudSave, 0));
    wrapFunction('deleteExpense');
  }
  ...
  function toggleItineraryEdit() {
    ...
    if (addDay) addDay.hidden = !editMode;
    window.renderItinerary?.();
  }
  ...
  async function initializeCloud() {
    ...
    try {
      ...
      if (data) {
        const decoded = decodeEntryPayload(data.entry_tasks);
        applyRemote(data);
        if (!decoded.hasBudget) await pushCloud();
      } else {
        await pushCloud();
      }
      ...
      channel = client.channel(`trip-${tripId}`)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'trip_shared_state', filter: `trip_id=eq.${tripId}` }, payload => {
          ...
          if (!decoded.hasBudget) setTimeout(pushCloud, 80);
        })
        .subscribe(status => {
  ...
  window.__tripCloudScheduleSave = scheduleCloudSave;
